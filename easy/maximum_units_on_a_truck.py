...
@pytest.mark.parametrize(
    "box_types,truck_size,expected_output",
    (
        ([[1, 3], [2, 2], [3, 1]], 4, 8),
        ([[5, 10], [2, 5], [4, 7], [3, 9]], 10, 91),
    )
)
def test_max_unites(box_types, truck_size, expected_output):
    assert Solution().maximumUnits(box_types, truck_size) == expected_output
