...
class Solution:
    ...
    Time: O(logn)
    Space: O(1)
    Simple solution using first element as min benchmark
    """
...
class Solution2:
    ...
    Time: O(logn)
    Space: O(1)
    Use right elemt to check whether there are smaller nums on the right
    """
    def findMin(self, nums: List[int]) -> int:
        l = 0
        r = len(nums) - 1
...
def test_search(solution, nums, expected_result):
