...
one letter anywhere in wordA without changing the order of the other characters
to make it equal to wordB.
For example, "abc" is a predecessor of "abac", while "cba" is not a predecessor
of "bcad". A word chain is a sequence of words [word1, word2, ..., wordk] with
k >= 1, where word1 is a predecessor of word2, word2 is a predecessor of word3,
    ...
    def longestStrChain(self, words: List[str]) -> int:
        ...
        for word in sorted(words, key=len, reverse=True):
            for i in range(len(word)):
                predcessor = word[:i] + word[i + 1:]
                if predcessor in dp:
                    dp[predcessor] = max(dp[predcessor], dp[word] + 1)
...
def test_longest_str_chain(words, expected_output):
    assert Solution().longestStrChain(words) == expected_output
