...
class Solution:
    """
    Use rolling sum and binary search to find the index.
    Time:
        init: O(n)
        pickIndex: logn

    Space: O(n) for rolling sum but can be optimized to O(1) if reuse input data

    """
    def __init__(self, w: List[int]):
        self.rol_sum = copy.deepcopy(w)
        for i in range(1, len(self.rol_sum)):
            self.rol_sum[i] += self.rol_sum[i - 1]

    def pickIndex(self) -> int:
        choice = random.randint(1, self.rol_sum[-1])

        l, r = 0, len(self.rol_sum) - 1

        while l < r:
            mid = (l + r) // 2
            if self.rol_sum[mid] >= choice:
                r = mid
            else:
                l = mid + 1
