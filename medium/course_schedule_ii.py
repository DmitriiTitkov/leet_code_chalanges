...
want to take course ai. For example, the pair [0, 1], indicates that to take
...
class Solution:
    def findOrder(self, numCourses: int, prerequisites: List[List[int]]) -> List[int]:
        adj_graph = {x: [[], 0] for x in range(numCourses)}
        for dependant, node in prerequisites:
            adj_graph[node][0].append(dependant)
            adj_graph[dependant][1] += 1

        queue = collections.deque()
        for node in adj_graph:
            if adj_graph[node][1] == 0:
        ...
        while queue:
            ...
            for node in adj_graph[cur_node][0]:
                adj_graph[node][1] -= 1
                if adj_graph[node][1] == 0:
                    queue.appendleft(node)
