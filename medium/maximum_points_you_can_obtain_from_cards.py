...
class Solution:
    """
    Calculate max for subarray from both ends.
    Time: O(n)
    Space: O(1)
    """
    def maxScore(self, cardPoints: List[int], k: int) -> int:
        left, right = k - 1, len(cardPoints)

        cur_score = sum(cardPoints[:k])
        max_score = cur_score

        while left >= 0:
            cur_score -= cardPoints[left]
            left -= 1

            right -= 1
            cur_score += cardPoints[right]
...
@pytest.mark.parametrize(
    "card_points,n,expected_output", (
        ([1, 2, 3, 4, 5, 6, 1], 3, 12),
        ([2, 2, 2], 2, 4),
        ([9, 7, 7, 9, 7, 7, 9], 7, 55),
    )
)
def test_max_score(card_points, n, expected_output):
    assert Solution().maxScore(card_points, n) == expected_output
