...
cannot change the direction of the taxi. The passengers are represented by a
0-indexed 2D integer array rides, where rides[i] = [starti, endi, tipi] denotes
the ith passenger requesting a ride from point starti to point endi who is
...
class Solution:
    ...
    def maxTaxiEarnings(self, n: int, rides: List[List[int]]) -> int:
        ...
        for i in range(1, n + 1):
            ...
            while curr_ride < len(rides) and rides[curr_ride][0] == i:
                start, end, tip = rides[curr_ride]
                trip_money = end - start + tip
                dp[end] = max(dp[end], trip_money + dp[i])
                curr_ride += 1
...
def test_max_taxi_earnings(n,rides,expected_output):
    assert Solution().maxTaxiEarnings(n, rides) == expected_output
